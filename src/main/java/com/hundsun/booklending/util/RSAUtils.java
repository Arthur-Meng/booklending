...
public abstract class RSAUtils {
	...
	private static synchronized KeyPair generateKeyPair() {
		byte[] random = new byte[1024];// DateFormatUtils.format(new
										// Date(),"yyyyMMdd").getBytes())
		new Random().nextBytes(random);
		try {
			keyPairGen.initialize(KEY_SIZE, new SecureRandom(random));
    ...
    public static void main(String[] args) {
        ...
        String publicKeyString = Base64.encodeBase64String(keys.getPublic().getEncoded());
        // 得到私钥字符串
        String privateKeyString = Base64.encodeBase64String(keys.getPrivate().getEncoded());
        System.out.println("publicKeyString:" + publicKeyString);
        System.out.println("privateKeyString:" + privateKeyString);

        System.out.println("--------------公钥加密私钥解密过程-------------------");
        String plainText = "D21DC8DA8E4D9A583A148F4B9411EBE8";
        // 公钥加密过程
        byte[] cipherData;
        try {
            cipherData = encrypt(loadPublicKeyByStr(publicKeyString), plainText.getBytes());
            String cipher = Base64.encodeBase64String(cipherData);
            System.out.println("加密后：" + cipher);
            // 私钥解密过程
            byte[] res = decrypt(loadPrivateKeyByStr(privateKeyString), Base64.decodeBase64(cipher));
